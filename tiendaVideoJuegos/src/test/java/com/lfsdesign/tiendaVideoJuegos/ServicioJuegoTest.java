...
public class ServicioJuegoTest {
    ...
    public void testAltaJuego() {
        ...
        Juego juego = new Juego(UUID.randomUUID(), "Nuevo Juego", 20.99, 2);
        int result = servicioJuego.altaJuego(juego);
        assertThat(result).isEqualTo(1);
        assertThat(juego.getId()).isNotNull();
    }

    @Test
    public void testObtenerTodosLosJuegos() {
    	
        when(daoJuego.selectAllJuegos()).thenReturn(juegos);
        ArrayList<Juego> result = servicioJuego.obtenerTodosLosJuegos();
    ...
    public void testBorrarJuego() {
        ...
        UUID id = juego1.getId();
        when(daoJuego.borrarJuegoById(id)).thenReturn(1);
        int result = servicioJuego.borrarJuego(id);
        assertThat(result).isEqualTo(1);
    }
